...
describe('LeadService.convert', () => {
  ...
  const sequences = {
    getNextNumber: jest.fn().mockResolvedValue('CUS-2026-00001'),
  };
  const tenantId = 't1';
  const userId = 'u1';
  ...
  it('rejects converting a lead that is not qualified', async () => {
    const tx = {
      lead: {
        findFirst: jest.fn().mockResolvedValue({ id: 'l1', status: 'new' }),
      },
    };
    prisma.$transaction.mockImplementation((fn: any) => fn(tx));
    await expect(
      service.convert(tenantId, userId, 'l1', {}),
  ...
  it('creates customer + opportunity and marks the lead won', async () => {
    const tx = {
      ...
      opportunity: {
        create: jest
          .fn()
          .mockImplementation((a: any) => ({ id: 'opp1', ...a.data })),
      },
    };
    prisma.$transaction.mockImplementation((fn: any) => fn(tx));
    ...
    expect(tx.customer.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ code: 'CUS-2026-00001', name: 'ACME' }),
      }),
    );
    expect(tx.opportunity.create).toHaveBeenCalled();
    expect(tx.lead.updateMany).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ status: 'won', customerId: 'cust1' }),
      }),
    );
  ...
  it('404s on an unknown lead', async () => {
    ...
    prisma.$transaction.mockImplementation((fn: any) => fn(tx));
    await expect(
      service.convert(tenantId, userId, 'missing', {}),
