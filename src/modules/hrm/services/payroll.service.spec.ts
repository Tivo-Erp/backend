...
describe('PayrollService', () => {
  ...
  describe('calculate', () => {
    it('refuses a duplicate run for the same period', async () => {
      const tx = {
        payrollRun: { findFirst: jest.fn().mockResolvedValue({ id: 'pr1' }) },
      };
    ...
    it('computes lines for all active employees and aggregates totals', async () => {
      const tx = {
        payrollRun: {
          ...
          create: jest
            .fn()
            .mockImplementation((a: any) => ({ id: 'pr1', ...a.data })),
        },
        employee: {
  ...
  describe('approve', () => {
    it('creates a balanced posted journal and marks approved', async () => {
      ...
      const totalDebit = callArg.entries.reduce(
        (s: number, e: any) => s + Number(e.debitAmount),
        0,
      );
      const totalCredit = callArg.entries.reduce(
        (s: number, e: any) => s + Number(e.creditAmount),
        0,
      );
      ...
      expect(tx.payrollRun.updateMany).toHaveBeenCalledWith(
        expect.objectContaining({
          data: { status: 'approved', journalBatchId: 'jb1' },
        }),
      );
    });
    ...
    it('rejects approving a non-draft run', async () => {
      const tx = {
        payrollRun: {
          findFirst: jest
            .fn()
            .mockResolvedValue({ id: 'pr1', status: 'approved' }),
        },
      };
      prisma.$transaction.mockImplementation((fn: any) => fn(tx));
      await expect(
        service.approve(tenantId, 'pr1', userId),
      ).rejects.toBeInstanceOf(ConflictException);
    });
  });
});
