...
describe('ItemService', () => {
  ...
  describe('create', () => {
    const dto: any = { sku: 'SKU-001', name: 'Widget', itemType: 'product', baseUom: 'PCS' };
    ...
    it('throws 400 when both isBatchTracked and isSerialTracked are true', async () => {
      const conflictDto = { ...dto, isBatchTracked: true, isSerialTracked: true };
      await expect(service.create(tenantId, conflictDto)).rejects.toThrow(BadRequestException);
  ...
  describe('findAll', () => {
    it('returns paginated response', async () => {
      repo.findAll.mockResolvedValue({ data: [{ id: itemId }], total: 1, page: 1, limit: 20 });
  ...
  describe('findOne', () => {
    ...
    it('throws 404 when item not found', async () => {
      ...
      await expect(service.findOne(tenantId, itemId, ['viewer'])).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('update', () => {
    it('updates item', async () => {
      ...
      const result = await service.update(tenantId, itemId, { name: 'Updated' } as any);
      expect(result.name).toBe('Updated');
    });

    it('throws 404 when item not found', async () => {
      repo.findById.mockResolvedValue(null);
      await expect(service.update(tenantId, itemId, {} as any)).rejects.toThrow(NotFoundException);
    });
    ...
    it('allows SKU update when it belongs to same item', async () => {
      ...
      const result = await service.update(tenantId, itemId, { sku: 'SKU-NEW' } as any);
    ...
    it('throws 400 on batch+serial conflict', async () => {
      repo.findById.mockResolvedValue({ id: itemId, sku: 'SKU-001' });
      await expect(
        service.update(tenantId, itemId, { isBatchTracked: true, isSerialTracked: true } as any),
  ...
  describe('remove', () => {
    ...
    it('throws 404 when item not found', async () => {
      repo.findById.mockResolvedValue(null);
      await expect(service.remove(tenantId, itemId)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('activate', () => {
    ...
    it('throws 400 when item is not in draft status', async () => {
      ...
      await expect(service.activate(tenantId, itemId)).rejects.toThrow(BadRequestException);
    });

    it('throws 404 when item not found', async () => {
      repo.findById.mockResolvedValue(null);
      await expect(service.activate(tenantId, itemId)).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('bulkImport', () => {
    ...
    it('imports all valid items in one batch call', async () => {
      ...
      const result = await service.bulkImport(tenantId, makeDto(items) as any);
    ...
    it('skips items with batch+serial conflict before batch call', async () => {
      const items = [
        { sku: 'A', isBatchTracked: true, isSerialTracked: true, name: 'A', itemType: 'product', baseUom: 'PCS' },
        { sku: 'B', name: 'B', itemType: 'product', baseUom: 'PCS' },
      ];
      repo.upsertBulk.mockResolvedValue([]);
