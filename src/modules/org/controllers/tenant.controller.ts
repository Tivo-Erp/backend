...
export class TenantController {
  ...
  @ApiOperation({
    ...
    description:
      'Self-service tenant registration. Creates tenant + first admin user + starter plan subscription.',
  })
  ...
  @ApiOperation({
    ...
    description: 'Get current tenant profile with Sparse Fieldsets support.',
  })
  @ApiQuery({
    name: 'fields',
    required: false,
  ...
  @ApiResponse({
    status: 200,
    description: 'Tenant profile with selected fields',
  })
  @ApiResponse({
    status: 401,
  ...
  @ApiOperation({
    ...
    description:
      'Update current tenant settings. Requires `org:tenant:update` permission.',
  })
  ...
  @ApiResponse({
    status: 403,
    description: 'Missing permission: org:tenant:update',
  })
  async updateProfile(
    @CurrentTenant() tenantId: string,
